...
def play():
    ...
    for i in range(max_round):
        guess = gui.integerbox('You have {0} chance(s) left! Please guess the number ({1}-{2}):'.format(
            max_round - i, lower_bound, upper_bound), title, None, lower_bound, upper_bound)
